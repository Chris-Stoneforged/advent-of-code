...
def calc_att(att, a, b):
    if att == "cats" or att == "trees": return a > b
    if att == "pomeranians" or att == "goldfish": return a < b
    return a == b
...
def run(source):
    ...
    for line in open(source):
        ...
        if all([k not in atts or calc_att(k, atts[k], v) for k, v in target.items()]):
